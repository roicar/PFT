...
void affine_transform_print(FILE *fp,PlutoMatrix *mat, PlutoProg *prog, char **vars) {
    int ndims = prog->nvar+prog->npar+1;  // Assuming 'nvar' refers to the number of affine transformations

    // // Variable names allocation and initialization
    // char *var[ndims];
    // for (int j = 0; j < ndims; j++) {
    //     if (vars && vars[j]) {
    //         var[j] = vars[j];  // Use provided name
    //     } else {
    //         var[j] = malloc(10);  // Allocate space for default name
    //         sprintf(var[j], "x%d", j + 1);  // Default name
    //     }
    // }

    // Process each row of the matrix
    for (int i = 0; i < mat->nrows; i++) {
        int first = 0;  // To handle the '+' sign before variables
        for (int j = 0; j < mat->ncols - 1; j++) {  // Last column is for the constant term
            ...
            if (coeff != 0) {
                if (coeff == 1) {
                    ...
                    fprintf(fp, "%s%lld%s", (coeff > 0 && first ? "+" : ""), coeff, vars[j]);
                }
                first = 1;  // Found at least one non-zero coefficient
            }
        }
        ...
        if (constant != 0) {
            ...
            if (!first) fprintf(fp, "0");  // If all coefficients were zero
        }
        fprintf(fp, " ");  // New line for the next transformation
    }

    // // Cleanup if memory was allocated
    // for (int j = 0; j < ndims; j++) {
    //     if (!vars || !vars[j]) {
    //         free(var[j]);  // Free only if it was malloc'ed here
    //     }
    // }
}
