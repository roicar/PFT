...
#define cudaCheckKernel() \
  do { \
    ...
    cudaCheckReturn(cudaMalloc((void **) &dev_tmp, (1024) * sizeof(float)));
    cudaCheckReturn(cudaMalloc((void **) &dev_x, (1024) * sizeof(float)));
    cudaCheckReturn(cudaMalloc((void **) &dev_y, (1024) * sizeof(float)));
    
    cudaCheckReturn(cudaMemcpy(dev_A, A, (1024) * (1024) * sizeof(float), cudaMemcpyHostToDevice));
    cudaCheckReturn(cudaMemcpy(dev_x, x, (1024) * sizeof(float), cudaMemcpyHostToDevice));
    {
      dim3 k0_dimBlock(32);
      dim3 k0_dimGrid(32);
    ...
    cudaCheckReturn(cudaMemcpy(tmp, dev_tmp, (1024) * sizeof(float), cudaMemcpyDeviceToHost));
    cudaCheckReturn(cudaMemcpy(y, dev_y, (1024) * sizeof(float), cudaMemcpyDeviceToHost));
