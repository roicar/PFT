...
#define cudaCheckKernel() \
  do { \
    ...
    cudaCheckReturn(cudaMalloc((void **) &dev_p, (1024) * sizeof(float)));
    cudaCheckReturn(cudaMalloc((void **) &dev_q, (1024) * sizeof(float)));
    cudaCheckReturn(cudaMalloc((void **) &dev_r, (1024) * sizeof(float)));
    cudaCheckReturn(cudaMalloc((void **) &dev_s, (1024) * sizeof(float)));
    
    cudaCheckReturn(cudaMemcpy(dev_A, A, (1024) * (1024) * sizeof(float), cudaMemcpyHostToDevice));
    cudaCheckReturn(cudaMemcpy(dev_p, p, (1024) * sizeof(float), cudaMemcpyHostToDevice));
    cudaCheckReturn(cudaMemcpy(dev_r, r, (1024) * sizeof(float), cudaMemcpyHostToDevice));
    {
      dim3 k0_dimBlock(32);
      dim3 k0_dimGrid(32);
    ...
    cudaCheckReturn(cudaMemcpy(q, dev_q, (1024) * sizeof(float), cudaMemcpyDeviceToHost));
    cudaCheckReturn(cudaMemcpy(s, dev_s, (1024) * sizeof(float), cudaMemcpyDeviceToHost));
