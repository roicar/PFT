...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
          ...
          cudaCheckReturn(cudaMalloc((void **) &dev_b, (1022) * (1024) * sizeof(float)));
          
          cudaCheckReturn(cudaMemcpy(dev_a, a, (1024) * (1024) * sizeof(float), cudaMemcpyHostToDevice));
          cudaCheckReturn(cudaMemcpy(dev_b, b, (1022) * (1024) * sizeof(float), cudaMemcpyHostToDevice));
          {
          ...
          cudaCheckReturn(cudaMemcpy(b, dev_b, (1022) * (1024) * sizeof(float), cudaMemcpyDeviceToHost));
