...
int main(){
    float A[1024][1024], B[1024][1024], C[1024][1024], D[1024][1024], E[1024][1024], F[1024][1024], G[1024][1024];
    FILE *fp = NULL;
    ...
    Init_Matrix_zero_PSxM(F, 1024,1024);
    Init_Matrix_zero_PSxM(G, 1024,1024);
...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
      ...
      cudaCheckReturn(cudaMalloc((void **) &dev_F, (1024) * (1024) * sizeof(float)));
      cudaCheckReturn(cudaMalloc((void **) &dev_G, (1024) * (1024) * sizeof(float)));
      ...
      cudaCheckReturn(cudaMemcpy(dev_F, F, (1024) * (1024) * sizeof(float), cudaMemcpyHostToDevice));
      cudaCheckReturn(cudaMemcpy(dev_G, G, (1024) * (1024) * sizeof(float), cudaMemcpyHostToDevice));
      {
      ...
      
      {
        ...
        kernel1 <<<k1_dimGrid, k1_dimBlock>>> (dev_D, dev_E, dev_F);
        cudaCheckKernel();
      }
      
      {
        dim3 k2_dimBlock(16, 32);
        dim3 k2_dimGrid(32, 32);
        kernel2 <<<k2_dimGrid, k2_dimBlock>>> (dev_C, dev_F, dev_G);
      ...
      cudaCheckReturn(cudaMemcpy(F, dev_F, (1024) * (1024) * sizeof(float), cudaMemcpyDeviceToHost));
      cudaCheckReturn(cudaMemcpy(G, dev_G, (1024) * (1024) * sizeof(float), cudaMemcpyDeviceToHost));
