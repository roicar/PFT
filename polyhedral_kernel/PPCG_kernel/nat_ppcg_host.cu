...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
      ...
      cudaCheckReturn(cudaMalloc((void **) &dev_B, (1023) * sizeof(float)));
      cudaCheckReturn(cudaMalloc((void **) &dev_C, (512) * sizeof(float)));
      
      cudaCheckReturn(cudaMemcpy(dev_A, A, (1024) * sizeof(float), cudaMemcpyHostToDevice));
      cudaCheckReturn(cudaMemcpy(dev_B, B, (1023) * sizeof(float), cudaMemcpyHostToDevice));
      {
        dim3 k0_dimBlock(32);
        dim3 k0_dimGrid(16);
      ...
      cudaCheckReturn(cudaMemcpy(C, dev_C, (512) * sizeof(float), cudaMemcpyDeviceToHost));
