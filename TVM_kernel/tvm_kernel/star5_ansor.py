...
def seidel_5(N):
    ...
    A_out = te.compute((N,N),lambda i,j:te.if_then_else(tvm.tir.all(i>=1,i<=N-2,j>=1,j<=N-2),( A[i-1][j]  + A[i][j-1] + A[i][j] + A[i][j+1]+ A[i+1][j] )/5,A[i][j]))
    return [A,A_out]
...
def mytuner(n_trial,N):
    ...
    dev = tvm.cpu()
    task = tvm.auto_scheduler.SearchTask(func=seidel_5, args=(N,), target=target)
    
    print("Computational DAG:")
    print(task.compute_dag)
    
    log_file = "seidel_5.json"
